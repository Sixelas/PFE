...
#####################################################################################
# Cloud Agent B - Agent in contact with the Indy ledger. 
# Issues credentials and asks Proof Requests

# The code for this agent was mostly inspired by the code available as a correction
# for the Acme Controller Workshop available at :
# https://github.com/hyperledger/aries-cloudagent-python/blob/main/demo/AcmeDemoWorkshop.md

# The original code was an incomplete controller. Can be found in /demo/runners/acme.py
# All of the parts we completed will be preceded by a comment of this type
#######################################################################################

# Define a credential preview type using a did
...
#######################################################################################
# Our VCs are made of the connection_id, the date and the timestamp
...
#######################################################################################
# This option requests a proof from the other Agent

            elif option == "2":
                log_status("#20 Request proof from alice")

                #define the attributes and the proof request
                req_attrs = [
                    {
                        "name": "publickey",
