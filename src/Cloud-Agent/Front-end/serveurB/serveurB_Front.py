...
InvitCommand = ''' curl -X POST "http://localhost:11000/out-of-band/create-invitation" -H 'Content-Type: application/json' -d '{ "handshake_protocols": ["did:sov:BzCbsNYhMrjHiqZDTUASHg;spec/didexchange/1.0"],"use_public_did": false}' > invitServeurW.json '''
...
# Fonction appelée quand on clique sur le bouton "Générer invitation"
    def GButton_3_command(self):
        ...
        Invitproc.wait()
        invitJson = loadJSON(selfFolderPath + "/invitServeurW.json") #Récupère l'invitation dans le fichier json.
